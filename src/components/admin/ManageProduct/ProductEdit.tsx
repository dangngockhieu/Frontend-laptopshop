...
const ProductEditContent = ({ setShow, product, onRefresh }: ProductEditContentProps) => {
  ...
  () => (Array.isArray(product?.features) ? product.features.map((f) => Number(f)) : []),
  [product]
);
  ...
  const handleSubmit = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      if (!(resUpdate && resUpdate.data && resUpdate.data.statusCode < 400)) {
      ...
      for (const id of removed) {
        const res =await deleteProductFeature(product.id, id);
