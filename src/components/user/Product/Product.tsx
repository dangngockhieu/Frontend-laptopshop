...
import type { ProductSummary, ProductFilters } from "../../../interfaces";
...
const Product = () => {
  ...
  const handleAddToCart = async (productId: number) => {
    ...
    try {
      ...
      if (res?.data?.statusCode<400) {
        ...
        if (cartRes?.data?.statusCode<400) dispatch(setCartCount(cartRes.data.data));
      } else {
  ...
  const handleBuyNow = async (productId: number) => {
    ...
    try {
      ...
      if (res?.data?.statusCode<400) {
        const cartRes = await getNumberCart();
        if (cartRes?.data?.data) dispatch(setCartCount(cartRes?.data?.data));
        navigate('/cart');
      } 
    } catch {
      toast.error("Đã xảy ra lỗi. Vui lòng thử lại sau.");
  ...
  const toggleAttributeSection = (key: string) => {
  ...
  const fetchProducts = async (
    ...
    try {
      // FIX 1: Truyền category: type vào thay vì object rỗng {}
      const res = await getFilteredProducts({ category: type });
      if (res?.data.statusCode<400) {
  ...
  const handleFilter = async () => {
    setVisibleCount(16);
    const processedFilters: SelectedFilters = { ...selectedFilters };
    
    // Map value properties to readable labels to send to API
    if (processedFilters.CPU?.length > 0) {
      const cpuOptions = LAPTOP_FILTERS.CPU.filter(isOptionObj);
      processedFilters.CPU = processedFilters.CPU.map((v: string | null) => {
        const found = cpuOptions.find((opt) => opt.name === v);
        return found ? found.label : v;
      });
    }

    if (processedFilters["PIN (mAh)"]?.length > 0) {
      const pinOptions = PHONE_FILTERS["PIN (mAh)"].filter(isOptionObj);
      processedFilters["PIN (mAh)"] = processedFilters["PIN (mAh)"].map((v: string | null) => {
        const found = pinOptions.find((opt) => opt.name === v);
        return found ? found.label : v;
      });
    }

    if (processedFilters["Cạc đồ họa rời"]?.length > 0) {
      const gpuOptions = LAPTOP_FILTERS["Cạc đồ họa rời"].filter(isOptionObj);
      processedFilters["Cạc đồ họa rời"] = processedFilters["Cạc đồ họa rời"].map((v: string | null) => {
        const found = gpuOptions.find((opt) => opt.name === v);
        return found ? found.label : v;
      });
    }

    if (processedFilters["Kích thước màn hình (inch)"]?.length > 0) {
      ...
      processedFilters["Kích thước màn hình (inch)"] = processedFilters["Kích thước màn hình (inch)"].map((v: string | null) => {
        const found = sizeOptions.find((opt) => opt.name === v);
        return found ? found.label : v;
      });
    }

    const cleanArray = <T,>(arr: (T | null)[] | undefined): T[] | undefined => {
      if (!arr) return undefined;
      const cleaned = arr.filter((v): v is T => v !== null && v !== "Tất cả" && v !== "all");
      return cleaned.length > 0 ? cleaned : undefined;
    };

    const factories = cleanArray(
      selectedFactories.map((id) => currentFactories.find((b) => b.id === id)?.name)
    ) as string[] | undefined;
      
    const productFeatures = cleanArray(
      selectedFeatures.map((id) => currentFeatures.find((f) => f.id === id)?.id)
    ) as number[] | undefined;
    ...
    const activePriceRange = customPriceRange ?? selectedPriceRange;

    // FIX 2: Bỏ chữ Partial đi và khai báo rõ `category` ở đây
    const filters: ProductFilters = {
      ...
      cpu: cleanArray(processedFilters["CPU"]),
      ram: cleanArray(processedFilters["RAM"]),
      gpu: cleanArray(processedFilters["Cạc đồ họa rời"]),
      storage: cleanArray(processedFilters["Bộ nhớ"]),
      screenSize: cleanArray(processedFilters["Kích thước màn hình (inch)"]),
      battery: cleanArray(processedFilters["PIN (mAh)"]),
      screen: cleanArray(processedFilters["Màn Hình"]),
    };
          ...
          <div className="filter-divider">
            ...
            {filterKeys.map((key) => (
              <div className="filter-item" key={key}>
                <div className="filter-title" onClick={() => toggleAttributeSection(key)}>
                  <label>{key}</label>
                  {expandedSections.attributes[key] ? <FaChevronUp /> : <FaChevronDown />}
                </div>
                {expandedSections.attributes[key] && (
                  <div className="option-grid">
