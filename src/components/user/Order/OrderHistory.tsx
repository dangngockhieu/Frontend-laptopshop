...
import { getMyOrders, updateOrderForUser, buyAgain, getNumberCart, createReview, createURLPayment } from '../../../services/apiServices.js';
...
const OrderHistory = () => {
    ...
    const handlePayment = async (orderId: number) => {
        try {
            const res = await createURLPayment(orderId);
            const paymentUrl = res?.data?.data?.paymentUrl;
    
            if (paymentUrl) {
            // Chuyển hướng người dùng tới lại VNPay
    ...
    const handleBuyAgain = async (products: ProductInOrder[]) => {
        try {
            const productDTOs = products.map(prod => ({ productId: prod.productId}));
            const res = await buyAgain(productDTOs);
            if (res?.data?.statusCode < 400) {
                                    ...
                                    <div className="card-body">
                                        ...
                                        {item.products.map(prod => (
                                            <div key={prod.productId} className="product-item">
                                                <img src={prod.imageUrl ? (prod.imageUrl.startsWith("/") ? `${BASE_URL}${prod.imageUrl}` : `${BASE_URL}/${prod.imageUrl}`) : '/no-image.png'} alt={prod.productName} className="item-img-preview"/>
                                                <div className="product-name">{prod.productName}</div>
                                                <div className="product-quantity">x{prod.quantity}</div>
                                                {item.status === 'COMPLETED' && !prod.reviewed &&(
                    ...
                    <div className="review-modal">
                        ...
                        <div className="review-item-info">
                            ...
                            <div className="ri-main">
                                <div className="ri-name">{reviewItem.productName}</div>
                            </div>
                        </div>
